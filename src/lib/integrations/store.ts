...
function fromEnv(): IntegrationsConfig {
  return {
    ...
    whatsapp: {
      ...
      bookingTemplateName: process.env.WHATSAPP_BOOKING_TEMPLATE_NAME ?? "",
      bookingTemplateLanguage:
        process.env.WHATSAPP_BOOKING_TEMPLATE_LANGUAGE ?? "en",
    },
    ai: {
...
export function toEnvSnippet(config: IntegrationsConfig): string {
  return [
    ...
    `WHATSAPP_BOOKING_TEMPLATE_NAME=${config.whatsapp?.bookingTemplateName ?? ""}`,
    `WHATSAPP_BOOKING_TEMPLATE_LANGUAGE=${config.whatsapp?.bookingTemplateLanguage ?? "en"}`,
